...
```{r,eval=F}
mkdir analysis_and_temp_files/05_genome_annotation
...
samtools faidx analysis_and_temp_files/05_genome_annotation/GTX0536.fasta
```
...
```{r}
gc<-read.delim2("../analysis_and_temp_files/05_genome_annotation/GTX0536.gc",header=T)
colnames(gc)[1]<-"contig"
gc$GC<-as.numeric(gc$GC)
cov<-read.delim2("../analysis_and_temp_files/05_genome_annotation/GTX0536.fasta.depth.txt")
colnames(cov)[1]<-"contig"
bins<-read.delim2("../analysis_and_temp_files/05_genome_annotation/GTX0536.cov")
...
blast_results<-read.delim2("../analysis_and_temp_files/05_genome_annotation/GTX0536.blast.out",header=F)
...
write.table(blast2, "../analysis_and_temp_files/05_genome_annotation/GTX0536.blast.taxonomy.txt",col.names = T, row.names = F, quote = F,sep="\t")
```
...
blast<-read.delim2("../analysis_and_temp_files/05_genome_annotation/GTX0536.blast.taxonomy.txt",header=T)
...
library(pafr)
paf<-read_paf("../analysis_and_temp_files/05_genome_annotation/GTX0536_SL0000003.paf")
...
* Saved the 20 contigs as `analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear.fasta`
```{r,eval=F}
write.table(align$qname[1:19], "../analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear.txt",col.names = F, row.names = F, quote = F,sep="\t")
source package 1041444f-cd25-4107-a5c7-5e86cb1728fe
seqkit grep -i -f analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear.txt analysis_and_temp_files/05_genome_annotation/GTX0536.fasta  > analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear.fasta 
```
...
* As a query used “CCCTAAA”, which is a telomeric repeat conserved between green algae and land plants (see Fulnekova et al. 2012)
  ...
  * 2 had none
```{r,eval=F}
python code/detect_telomers.py analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear.fasta -m "CCCTAAA" > analysis_and_temp_files/05_genome_annotation/GTX0536_telomere_detection.txt
```

* Used SEQtk and got GC% for non-overlapping windows of 1000 bp
```{r,eval=F}
source package 46a62eca-4f8f-45aa-8cc2-d4efc99dd9c6
seqkit sliding  analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear.fasta -s 1000 -W 1000  | seqkit fx2tab -n -g > analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear.fasta_GC_sliding.txt
```
...
library(stringr)
library(viridis)
gc<-read.delim2("../analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear.fasta_GC_sliding.txt",header=F)[,c(1,2)]
...
tel<-read.delim2("../analysis_and_temp_files/05_genome_annotation/GTX0536_telomere_detection.txt",header=F)[,c(1,2)]
...
```{r, out.width = '80%'}
knitr::include_graphics("../analysis_and_temp_files/05_genome_annotation/checking_for_misassemblies/ptg000019l.png")
```
...
```{r,eval=F}
for i in {2750..3500}; do s=$(($i*1000)); e=$(($i*1000+1000)); samtools coverage -r ptg000002l:$s-$e analysis_and_temp_files/05_genome_annotation/GTX0536.bam; done > analysis_and_temp_files/05_genome_annotation/ptg000002l.cov
grep "ptg000002l" analysis_and_temp_files/05_genome_annotation/ptg000002l.cov > analysis_and_temp_files/05_genome_annotation/ptg000002l.filtered.cov
```
* Coverage is uneven here, but there are no clear changes in GC%. The window with the lowest coverage still shows to ha
```{r}
depth<-read.delim2("../analysis_and_temp_files/05_genome_annotation/ptg000002l.filtered.cov",header=F)[,c(2,7)]
...
```{r, out.width = '80%'}
knitr::include_graphics("../analysis_and_temp_files/05_genome_annotation/checking_for_misassemblies/ptg000002.png")
```

* Can we find telomeres in this region?
  * Nope! got a few hits, but those are not clustered and are likely by chance
```{r,eval=F}
samtools faidx analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear.fasta ptg000002l:2750000-3500000 > analysis_and_temp_files/05_genome_annotation/ptg000002l_suspect.fa

grep "CCCTAAACCCTAAA" -n analysis_and_temp_files/05_genome_annotation/ptg000002l_suspect.fa
grep "GGGTTTAGGGTTT" -n analysis_and_temp_files/05_genome_annotation/ptg000002l_suspect.fa
```
* Can we find telomeric repeats in the middle of other contigs? Searched the entire assembly
```{r,eval=F}
seqkit locate -i -p "CCCTAAACCCTAAACCCTAAACCCTAAACCCTAAA" analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear.fasta >  analysis_and_temp_files/05_genome_annotation/GTX0536_telomeric_repeats_across.txt
```
...
repeats<-read.delim2("../analysis_and_temp_files/05_genome_annotation/GTX0536_telomeric_repeats_across.txt",header=T)
...
```{r, out.width = '80%'}
knitr::include_graphics("../analysis_and_temp_files/05_genome_annotation/checking_for_misassemblies/ptg000003l_3704286-3708286.png")
```

* From ptg000004l remove bases after 3027428
```{r, out.width = '80%'}
knitr::include_graphics("../analysis_and_temp_files/05_genome_annotation/checking_for_misassemblies/ptg000004l_3026090-3030090.png")
```

* All other matches to telomeric repeats were not associated with assembly breaks  `analysis_and_temp_files/05_genome_annotation/checking_for_misassemblies/`
...
```{r, out.width = '80%'}
knitr::include_graphics("../analysis_and_temp_files/05_genome_annotation/checking_for_misassemblies/ptg000008l_1-4000.png")
```

* Will remove bases past 4,931,045 from ptg000016l
```{r, out.width = '80%'}
knitr::include_graphics("../analysis_and_temp_files/05_genome_annotation/checking_for_misassemblies/ptg000016l_4929045-4931045.png")
```

* Will remove bases past 2,339,172 ptg000020l and before 1071
```{r, out.width = '80%'}
knitr::include_graphics("../analysis_and_temp_files/05_genome_annotation/checking_for_misassemblies/ptg000020l_1-4000.png")
knitr::include_graphics("../analysis_and_temp_files/05_genome_annotation/checking_for_misassemblies/ptg000020l_2335936-2339936.png")
```
...
```{r,eval=F}
samtools faidx analysis_and_temp_files/05_genome_annotation/GTX0536.fasta ptg000003l:3706236-3719598 > analysis_and_temp_files/05_genome_annotation/ptg000003l_tail.fa 
blastn -query analysis_and_temp_files/05_genome_annotation/ptg000003l_tail.fa  -subject data/SL0000003.fasta -outfmt 6

minimap2 -x asm20 -t 10 --secondary=no data/SL0000003.fasta analysis_and_temp_files/05_genome_annotation/ptg000003l_tail.fa > analysis_and_temp_files/05_genome_annotation/ptg000003l_tail_SL0000003.paf
...
minimap2 -x asm20 -t 10 analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear_final.fasta analysis_and_temp_files/05_genome_annotation/ptg000003l_tail.fa
...
```{r,eval=F}
seqkit grep -i -f analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear_final.txt analysis_and_temp_files/05_genome_annotation/GTX0536.fasta  > analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear_final.fasta 

samtools faidx analysis_and_temp_files/05_genome_annotation/GTX0536.fasta ptg000019l:1-2410725 >> analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear_final.fasta 

samtools faidx analysis_and_temp_files/05_genome_annotation/GTX0536.fasta ptg000003l:1-3706235 >> analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear_final.fasta 

samtools faidx analysis_and_temp_files/05_genome_annotation/GTX0536.fasta ptg000004l:1-3027428 >> analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear_final.fasta 

samtools faidx analysis_and_temp_files/05_genome_annotation/GTX0536.fasta ptg000008l:1035-2212220 >> analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear_final.fasta 

samtools faidx analysis_and_temp_files/05_genome_annotation/GTX0536.fasta ptg000016l:1-4931045 >> analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear_final.fasta 

samtools faidx analysis_and_temp_files/05_genome_annotation/GTX0536.fasta ptg000020l:1071-2339172 >> analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear_final.fasta 
...
```{r,eval=F}
cp analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear_final.fasta analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear_final2.fasta 

source package aeee87c4-1923-4732-aca2-f2aff23580cc
samtools faidx analysis_and_temp_files/05_genome_annotation/GTX0536.fasta ptg000023l >> analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear_final2.fasta 

```
* Cleaned and sorted the assembly using funannotate
```{r,eval=F}
singularity run ../singularity/funannotate.sif funannotate sort -i analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear_final2.fasta -o analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear_final_sort2.fasta -b GTX0536
```
...
```{r,eval=F}
mkdir analysis_and_temp_files/05_genome_annotation/GTX0536_busco2
...
busco -i analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear_final2.fasta -o GTX0536 --out_path analysis_and_temp_files/05_genome_annotation/GTX0536_busco2  -m  genome -l /tsl/data/busco_lineages/chlorophyta_odb10 --offline -f -c 20
```

* For comparison, analyzed the ASG genome, and got slightly worse results
  * C:92.3%[S:91.6%,D:0.7%],F:0.5%,M:7.2%,n:1519 
```{r,eval=F}
busco -i data/SL0000003.fasta -o SL0000003 --out_path analysis_and_temp_files/05_genome_annotation/SL0000003_busco  -m  genome -l /tsl/data/busco_lineages/chlorophyta_odb10 --offline -f -c 20
```
...
busco1<-read.delim("../analysis_and_temp_files/05_genome_annotation/GTX0536_busco/GTX0536/short_summary.specific.chlorophyta_odb10.GTX0536.txt",header=F,skip = 9)
...
b1<-ggplot(busco1,aes(y=V2,fill=type,x=hsize))+geom_col()+
  ...
  scale_fill_manual(values=c("Complete (95.3%)"="#60ba30","Duplicated (0.5%)"="#1f5900","Fragmented (0.7%)"="#ccf2b8","Missing (3.5%)"="white"))+
  theme_void()+theme(legend.title = element_blank(),legend.text = element_text(size=9),
                     legend.key=element_rect(colour="#969696"))+
  ggtitle("GTX0536")

busco2<-read.delim("../analysis_and_temp_files/05_genome_annotation/SL0000003_busco/SL0000003/short_summary.specific.chlorophyta_odb10.SL0000003.txt",header=F,skip = 9)
...
```{r,eval=F}
python code/detect_telomers.py analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear_final_sort2.fasta -m "CCCTAAA" > analysis_and_temp_files/05_genome_annotation/GTX0536_final_telomere_detection2.txt

seqkit sliding  analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear_final_sort2.fasta -s 1000 -W 1000  | seqkit fx2tab -n -g > analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear_final.fasta_GC_sliding2.txt
```

* Now, got both telomeres on 16 contigs and singe telomere on the remaining 3
```{r,fig.height=8,fig.width=8}
gc<-read.delim2("../analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear_final.fasta_GC_sliding2.txt",header=F)[,c(1,4)]
colnames(gc)<-c("window","gc_content")
##get contig name and start of the window
gc$contig<-sub("_sliding.*", "", gc$window)  
gc$window<-sub(".*:", "", gc$window)
...
##add data for telomere annotation
tel<-read.delim2("../analysis_and_temp_files/05_genome_annotation/GTX0536_final_telomere_detection2.txt",header=F)[,c(1,2)]
colnames(tel)<-c("contig","position")
tel_start_contig_list<-tel[tel$position=="forward",1] #list all contigs that have telomer at the contig start (corresponds to 'forward')
tel_end_contig_list<-tel[tel$position=="reverse",1] #list all contigs that have telomer at the contig start (corresponds to 'forward')
...
```{r,eval=F}
minimap2 -x asm20 -t 10 analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear_final_sort2.fasta data/SL0000003.fasta > analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear_final2_SL0000003.paf
```
* Get info on contig lengths for both files
```{r,eval=F}
seqkit fx2tab --length --name --header-line  data/SL0000003.fasta > analysis_and_temp_files/05_genome_annotation/SL0000003.lengths.txt

seqkit fx2tab --length --name --header-line  analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear_final_sort2.fasta > analysis_and_temp_files/05_genome_annotation/GTX0536.2.lengths.txt
```
...
#prep inputs
paf2<-read_paf("../analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear_final2_SL0000003.paf")
...
write.table(paf2_filtered,"../analysis_and_temp_files/05_genome_annotation/synteny_paf.txt",quote = F, col.names = F, row.names = F,sep="\t")

size1 <- read.delim("../analysis_and_temp_files/05_genome_annotation/GTX0536.2.lengths.txt")[,c(1,4)] %>% mutate(id="GTX0536")
size2 <- read.delim("../analysis_and_temp_files/05_genome_annotation/SL0000003.lengths.txt") %>% mutate(id="SL0000003")
...
write.table(size,"../analysis_and_temp_files/05_genome_annotation/synteny_size.txt",quote = F, col.names = F, row.names = F,sep="\t")

draw.linear.2.0("synteny",
            "../analysis_and_temp_files/05_genome_annotation/synteny_size.txt",
            "../analysis_and_temp_files/05_genome_annotation/synteny_paf.txt",
...
BuildDatabase -name analysis_and_temp_files/05_genome_annotation/GTX0536_repeatdb analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear_final_sort.fasta

RepeatModeler -database analysis_and_temp_files/05_genome_annotation/GTX0536_repeatdb -pa 20 -LTRStruct >& analysis_and_temp_files/05_genome_annotation/GTX0536_repeatmodeler.out

sbatch --mem=100G -c 32 --partition="tsl-long" --wrap="RepeatModeler -database analysis_and_temp_files/05_genome_annotation/GTX0536_repeatdb -pa 20 -LTRStruct >& analysis_and_temp_files/05_genome_annotation/GTX0536_repeatmodeler2.out"
```
* Repeat mask
```{r,eval=F}
source package /tsl/software/testing/bin/repeatmasker-4.0.9 
RepeatMasker -pa 5 -a -s -gff -xsmall -lib analysis_and_temp_files/05_genome_annotation/GTX0536_repeatdb-families.fa analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear_final_sort2.fasta &> analysis_and_temp_files/05_genome_annotation/repeatmasker_GTX0536.2.run.out
```
```
...
mkdir -p analysis_and_temp_files/05_genome_annotation/GTX0536_genemark2
cd analysis_and_temp_files/05_genome_annotation/GTX0536_genemark2
...
* Funannotate train
```{r,eval =F}
sbatch --mem=120G -c 28 --partition="nbi-long" --wrap="singularity run ../singularity/funannotate.sif funannotate train -i analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear_final_sort2.fasta.masked -o analysis_and_temp_files/05_genome_annotation/GTX0536_pred2 --left data/RNA_seq/X204SC25014007-Z01-F001/01.RawData/GTX0532/GTX0532_EKRN250006470-1A_22MTC7LT4_L8_1.fq.gz --right data/RNA_seq/X204SC25014007-Z01-F001/01.RawData/GTX0532/GTX0532_EKRN250006470-1A_22MTC7LT4_L8_2.fq.gz --jaccard_clip --cpus 28 --memory 120G --species 'Trebouxia sp. A48'"
```

* Funannotate predict
```{r,eval =F}
sbatch --mem=120G -c 28 --partition="nbi-long" --wrap="singularity run ../singularity/funannotate.sif funannotate predict -i analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear_final_sort2.fasta.masked  -o analysis_and_temp_files/05_genome_annotation/GTX0536_pred2 -s 'Trebouxia sp. A48' --cpus 28 --optimize_augustus --genemark_gtf analysis_and_temp_files/05_genome_annotation/GTX0536_genemark2/genemark.gtf --organism other --busco_db chlorophyta_odb10 -d /tsl/scratch/gol22pin/singularity/funannotate2/opt/databases --busco_seed_species chlamydomonas --weights genemark:1"
```
```
  Augustus       1        3816
  Augustus HiQ   2        7321
...
* Funannotate update
```{r,eval =F}
sbatch --mem=120G -c 28 --partition="nbi-long" --wrap="singularity run ../singularity/funannotate.sif funannotate update -i analysis_and_temp_files/05_genome_annotation/GTX0536_pred2 --cpus 28"
```

### 2.3. Functional annotations
* InterPro
```{r,eval =F}
mkdir -p analysis_and_temp_files/05_genome_annotation/GTX0536_pred2/interpro
rm -rf temp
...
sbatch --mem=60G -c 20 --partition="tsl-medium" --wrap="source package 0dd71e29-8eb1-4512-b37c-42f7158718f4; source package /tsl/software/testing/bin/gcc-5.2.0; source package 999eb878-6c39-444e-a291-e2e0a86660e6; source package /tsl/software/testing/bin/java-11.0.7; source package 0f2514dd-8288-47ed-96cd-80905f9b0644; source package /tsl/software/production/bin/perl-5.16.2; source package 0dd71e29-8eb1-4512-b37c-42f7158718f4; interproscan.sh -i analysis_and_temp_files/05_genome_annotation/GTX0536_pred2/update_results/Trebouxia_sp._A48.proteins.fa  -d analysis_and_temp_files/05_genome_annotation/GTX0536_pred2/interpro -dp -f XML -goterms -dra -cpu 20"
```

* Funannotate annotate
```{r,eval =F}
sbatch --mem=50G -c 20 --partition="tsl-medium" --wrap="singularity run ../singularity/funannotate.sif funannotate annotate -i analysis_and_temp_files/05_genome_annotation/GTX0536_pred2/ --iprscan analysis_and_temp_files/05_genome_annotation/GTX0536_pred2/interpro/Trebouxia_sp._A48.proteins.fa.xml --cpus 20  --sbt analysis_and_temp_files/05_genome_annotation/template.sbt --busco_db /tsl/data/busco_lineages/chlorophyta_odb10  --rename GTX0536PRED"
```
```
...
signalp -fasta analysis_and_temp_files/05_genome_annotation/GTX0536_pred2/annotate_results/Trebouxia_sp._A48.proteins.fa -org euk -format short -batch 50000 
mv Trebouxia_sp._A48.proteins_summary.signalp5 analysis_and_temp_files/05_genome_annotation/GTX0536_pred2/
```
* Extracted the list of all 745 proteins with predicted SP and save them as a separate fasta. had to split in two chunks
```{r,eval=F}
library(Biostrings)
signal<-read.delim2("../analysis_and_temp_files/05_genome_annotation/GTX0536_pred2/Trebouxia_sp._A48.proteins_summary.signalp5",skip=1)
signal_list<-signal$X..ID[signal$Prediction!="OTHER"]

fa <- readAAStringSet("../analysis_and_temp_files/05_genome_annotation/GTX0536_pred2/secretome/Trebouxia_sp._A48.proteins.fa")
names(fa) <- sub("\\s.*", "",names(fa))
fa_signal <- fa[names(fa) %in% signal_list]
writeXStringSet(fa_signal[1:372],"../analysis_and_temp_files/05_genome_annotation/GTX0536_pred2/secretome/Trebouxia_sp._A48.signalp1.fa")
writeXStringSet(fa_signal[373:745],"../analysis_and_temp_files/05_genome_annotation/GTX0536_pred2/secretome/Trebouxia_sp._A48.signalp2.fa")
```
* Fed the new fasta into DeepTMHMM
* Prepped files to get proteins with/without transmembrane domain and with a signal
```{r,eval=F}
grep "Number of predicted TMRs" analysis_and_temp_files/05_genome_annotation/GTX0536_pred2/secretome/TMRs*.gff3 -h > analysis_and_temp_files/05_genome_annotation/GTX0536_pred2/secretome/TMR_summary.txt

grep "signal" analysis_and_temp_files/05_genome_annotation/GTX0536_pred2/secretome/TMRs*.gff3 -h > analysis_and_temp_files/05_genome_annotation/GTX0536_pred2/secretome/deepTMHMM_signal.txt
```
...
```{r}
tm<-read.delim2("../analysis_and_temp_files/05_genome_annotation/GTX0536_pred2/secretome/TMR_summary.txt",header=F,sep=" ") %>% select(V2,V7)
colnames(tm)<-c("prot","TM_domains")

tm_signal<-read.delim2("../analysis_and_temp_files/05_genome_annotation/GTX0536_pred2/secretome/deepTMHMM_signal.txt",header=F,sep="\t") %>% select(V1,V2)
...
writeXStringSet(fa_deep,"../analysis_and_temp_files/05_genome_annotation/GTX0536_pred2/secretome/Trebouxia_sp._A48.signalp_deepTMHMM.fa")
```

* Ran WolfPSORT 
```{r,eval=F}
source package 666e3cc4-643e-4667-9235-fe054b436bfd
runWolfPsortSummary plant < analysis_and_temp_files/05_genome_annotation/GTX0536_pred2/secretome/Trebouxia_sp._A48.signalp_deepTMHMM.fa > analysis_and_temp_files/05_genome_annotation/GTX0536_pred2/secretome/wolfpsort.out
```
* Got 103 proteins with extracellular as the main prediction
```{r}
wolf<-read.delim2("../analysis_and_temp_files/05_genome_annotation/GTX0536_pred2/secretome/wolfpsort.out",skip = 1,header=F)
colnames(wolf)<-'V1'
...
writeXStringSet(fa_secreted,"../analysis_and_temp_files/05_genome_annotation/GTX0536_pred2/secretome/Trebouxia_sp._A48.secretome.fa")

ann <- read.delim2("../analysis_and_temp_files/05_genome_annotation/GTX0536_pred2/Trebouxia_sp._A48.annotations.reduced.txt") 
...
```{r}
ortho_genes<-read.delim2("../analysis_and_temp_files/06_phylogeny/orthofinder_input/Orthogroups.tsv")
ann <- read.delim2("../analysis_and_temp_files/05_genome_annotation/GTX0536_pred2/Trebouxia_sp._A48.annotations.reduced.txt")
...
* Extracted the contig and saved as `analysis_and_temp_files/05_genome_annotation/GTX0536_plastid.fa`
```{r,eval=F}
source package aeee87c4-1923-4732-aca2-f2aff23580cc
samtools faidx analysis_and_temp_files/05_genome_annotation/GTX0536.fasta ptg000005c > analysis_and_temp_files/05_genome_annotation/GTX0536_plastid.fa
```
...
source package aeee87c4-1923-4732-aca2-f2aff23580cc
mkdir analysis_and_temp_files/05_genome_annotation/GTX0536_plastid_STAR_index -p
STAR --runThreadN 10  --genomeSAindexNbases 6 \
--runMode genomeGenerate \
--genomeDir analysis_and_temp_files/05_genome_annotation/GTX0536_plastid_STAR_index \
--genomeFastaFiles analysis_and_temp_files/05_genome_annotation/GTX0536_plastid.fa
...
analysis_and_temp_files/05_genome_annotation/GTX0536_plastid_STAR_index/touch 20 analysis_and_temp_files/05_genome_annotation/GTX0536_plastid_GTX0532.bam"

samtools index analysis_and_temp_files/05_genome_annotation/GTX0536_plastid_GTX0532Aligned.sortedByCoord.out.bam
samtools faidx analysis_and_temp_files/05_genome_annotation/GTX0536_plastid.fa
```
...
mfannot analysis_and_temp_files/05_genome_annotation/GTX0536_plastid.fa 
mkdir analysis_and_temp_files/05_genome_annotation/GTX0536_plastid_tmp
mv GTX0536_plastid.fa* analysis_and_temp_files/05_genome_annotation/GTX0536_plastid_tmp

agat_convert_mfannot2gff.pl -i analysis_and_temp_files/05_genome_annotation/GTX0536_plastid_tmp/GTX0536_plastid.fa.new -o analysis_and_temp_files/05_genome_annotation/GTX0536_plastid_tmp/GTX0536_plastid_mfannot.gff
```
...
* Manually de-duplicated the gff file, since it has same gene annotated multiple times by multiple tools
  * Saved the file as `analysis_and_temp_files/05_genome_annotation/GTX0536_plastid.gff`
* Visualized using OGDRAW and saved on the `results`
```{r, out.width = '80%'}
knitr::include_graphics("../analysis_and_temp_files/05_genome_annotation/GTX0536_plastid_tmp/job-results-2025328201839/GeSeqJob-20250305-191553_ptg000005c_OGDRAW.jpg")
```
...
write.table(mitocontigs,"../analysis_and_temp_files/05_genome_annotation/GTX0536_mitocontigs.bed",quote = F, col.names = F, row.names = F,sep="\t")
```
* Extracted reads that mapped to these contigs
```{r,eval=F}
samtools view -b -L analysis_and_temp_files/05_genome_annotation/GTX0536_mitocontigs.bed analysis_and_temp_files/05_genome_annotation/GTX0536.bam > analysis_and_temp_files/05_genome_annotation/GTX0536_mitocontigs.bam

samtools fastq analysis_and_temp_files/05_genome_annotation/GTX0536_mitocontigs.bam > analysis_and_temp_files/05_genome_annotation/GTX0536_mitocontigs.fastq
```
...
```{r,eval=F}
head analysis_and_temp_files/05_genome_annotation/GTX0536_mitocontigs.fastq -n 120000 > analysis_and_temp_files/05_genome_annotation/GTX0536_mitocontigs_subset.fastq
gzip analysis_and_temp_files/05_genome_annotation/GTX0536_mitocontigs_subset.fastq

rm analysis_and_temp_files/05_genome_annotation/GTX0536_mitocontigs.bam
rm analysis_and_temp_files/05_genome_annotation/GTX0536_mitocontigs.fastq
```
* Assembled the reads with Hifiasm v0.18.5
```{r,eval=F}
sbatch --mem=100G -c 20 --wrap="source package 3c087633-18b1-4e77-9b08-7e68657fce66; hifiasm -o analysis_and_temp_files/05_genome_annotation/GTX0536_mito_reassembly -t 20 analysis_and_temp_files/05_genome_annotation/GTX0536_mitocontigs_subset.fastq.gz"
```
* Get the assembly in fasta from the assembly graph generated by hifiasm
  * This generates one circular and 5 linear contigs
```{r,eval=F}
awk '/^S/{print ">"$2;print $3}' analysis_and_temp_files/05_genome_annotation/GTX0536_mito_reassembly.bp.p_ctg.gfa > analysis_and_temp_files/05_genome_annotation/GTX0536_mito_reassembly.fa 
```
* Calculate lengths:
  * ptg000001c      109484
  * ptg000002l      11275
  ...
  * ptg000006l      52202
```{r,eval=F}
seqkit fx2tab --length --name --header-line  analysis_and_temp_files/05_genome_annotation/GTX0536_mito_reassembly.fa 
```
* **Got our mitochondria!** Save the contig ptg000001c as `analysis_and_temp_files/05_genome_annotation/GTX0536_mito.fa`
```{r,eval=F}
samtools faidx analysis_and_temp_files/05_genome_annotation/GTX0536_mito_reassembly.fa ptg000001c > analysis_and_temp_files/05_genome_annotation/GTX0536_mito.fa
```

## 4.2. Annotate
* Aligned RNA data onto it
```{r,eval=F}
mkdir analysis_and_temp_files/05_genome_annotation/GTX0536_mito_STAR_index -p
STAR --runThreadN 10  --genomeSAindexNbases 6 \
--runMode genomeGenerate \
--genomeDir analysis_and_temp_files/05_genome_annotation/GTX0536_mito_STAR_index \
--genomeFastaFiles analysis_and_temp_files/05_genome_annotation/GTX0536_mito.fa

sbatch --mem=100G -c 20 --wrap="code/star_align.sh  \
data/RNA_seq/X204SC25014007-Z01-F001/01.RawData/GTX0532/GTX0532_EKRN250006470-1A_22MTC7LT4_L8_1.fq.gz \
data/RNA_seq/X204SC25014007-Z01-F001/01.RawData/GTX0532/GTX0532_EKRN250006470-1A_22MTC7LT4_L8_2.fq.gz \
analysis_and_temp_files/05_genome_annotation/GTX0536_mito_STAR_index/touch 20 analysis_and_temp_files/05_genome_annotation/GTX0536_mito_GTX0532.bam"

samtools index analysis_and_temp_files/05_genome_annotation/GTX0536_mito_GTX0532Aligned.sortedByCoord.out.bam
samtools faidx analysis_and_temp_files/05_genome_annotation/GTX0536_mito.fa
```
* Used MFannot for annotation; used a script within AGAT to tranform the sqn output file into gff
```{r,eval=F}
source package d6c33ed3-e5cf-4826-b683-023f6b592f0b
source package 4c883633-af2d-4fac-ab67-a1574f7fe079
mfannot analysis_and_temp_files/05_genome_annotation/GTX0536_mito.fa 
mkdir analysis_and_temp_files/05_genome_annotation/GTX0536_mito_tmp/
mv GTX0536_mito.fa* analysis_and_temp_files/05_genome_annotation/GTX0536_mito_tmp

agat_convert_mfannot2gff.pl -i analysis_and_temp_files/05_genome_annotation/GTX0536_mito_tmp/GTX0536_mito.fa.new -o analysis_and_temp_files/05_genome_annotation/GTX0536_mito_tmp/GTX0536_mito_mfannot.gff
```
...
* Compared the annotations side by side with RNA alignments. The annotations are very similar. This time, MFannot seem to be much better in annotation rRNA. Manually made a combined annotation using GeSeq gff as the base, and added relevant lines from Mfannot there
  * Manually de-duplicated the gff file, since it has same gene annotated multiple times by multiple tools
  * Saved the file as `analysis_and_temp_files/05_genome_annotation/GTX0536_mito.gff`
* Visualized using OGDRAW and saved on the `results`
```{r, out.width = '80%'}
knitr::include_graphics("../analysis_and_temp_files/05_genome_annotation/GTX0536_mito_tmp/GTX0536_mito.png")
```
...
```{r,eval=F}
python code/detect_telomers.py data/SL0000003.fasta -m "CCCTAAA" > analysis_and_temp_files/05_genome_annotation/SL0000003_telomere_detection.txt

seqkit sliding  data/SL0000003.fasta -s 1000 -W 1000  | seqkit fx2tab -n -g > analysis_and_temp_files/05_genome_annotation/SL0000003.fasta_GC_sliding.txt
```

* Here, out of 20 contigs: 3 got both telomeres, 10 got a singe telomere, and 7 got no telomeres
```{r}
gc<-read.delim2("../analysis_and_temp_files/05_genome_annotation/SL0000003.fasta_GC_sliding.txt",header=F)[,c(1,2)]
colnames(gc)<-c("window","gc_content")
##get contig name and start of the window
gc$contig<-sub("_sliding.*", "", gc$window)  
gc$window<-sub(".*:", "", gc$window)
...
##add data for telomere annotation
tel<-read.delim2("../analysis_and_temp_files/05_genome_annotation/SL0000003_telomere_detection.txt",header=F)[,c(1,2)]
colnames(tel)<-c("contig","position")
...
```{r,eval=F}
source package 85eb6fb0-3eb7-43b2-9659-49e0142481fc
BuildDatabase -name analysis_and_temp_files/05_genome_annotation/SL0000003_repeatdb data/SL0000003.fasta

sbatch --mem=40G -c 20 --partition="tsl-long" --wrap="source package 85eb6fb0-3eb7-43b2-9659-49e0142481fc; RepeatModeler -database analysis_and_temp_files/05_genome_annotation/SL0000003_repeatdb -pa 20 -LTRStruct >& analysis_and_temp_files/05_genome_annotation/SL0000003_repeatmodeler.out"
```
* Repeat mask
```{r,eval=F}
source package /tsl/software/testing/bin/repeatmasker-4.0.9 
RepeatMasker -pa 5 -a -s -gff -xsmall -lib analysis_and_temp_files/05_genome_annotation/SL0000003_repeatdb-families.fa data/SL0000003.fasta &> analysis_and_temp_files/05_genome_annotation/repeatmasker_SL0000003.run.out
```
```
==================================================
...
mkdir -p analysis_and_temp_files/05_genome_annotation/SL0000003_genemark
cd analysis_and_temp_files/05_genome_annotation/SL0000003_genemark
...
```{r,eval =F}
sbatch --mem=50G -c 20 --partition="tsl-long" --wrap="singularity run ../singularity/funannotate.sif funannotate predict -i data/SL0000003.fasta.masked.renamed -o analysis_and_temp_files/05_genome_annotation/SL0000003_pred --species 'Trebouxia SL0000003' --cpus 20 --strain SL0000003 --optimize_augustus --genemark_gtf analysis_and_temp_files/05_genome_annotation/SL0000003_genemark/genemark.gtf --organism other --busco_db chlorophyta_odb10 -d /tsl/scratch/gol22pin/singularity/funannotate2/opt/databases --busco_seed_species chlamydomonas --weights genemark:1 --protein_evidence ../singularity/funannotate2/opt/databases/uniprot_sprot.fasta analysis_and_temp_files/05_genome_annotation/GTX0536_pred/predict_results/Trebouxia_sp._A48.proteins.fa"
...
mkdir -p analysis_and_temp_files/05_genome_annotation/SL0000003_pred/interpro
rm -rf temp

#tried newer version InterProScan-5.52-86.0
#CDD-3.18,Coils-2.2.1,Gene3D-4.3.0,Hamap-2020_05,MobiDBLite-2.0,PANTHER-15.0,Pfam-33.1,PIRSF-3.10,PIRSR-2021_02,PRINTS-42.0,ProSitePatterns-2021_01,ProSiteProfiles-2021_01,SFLD-4,SMART-7.1,SUPERFAMILY-1.75,TIGRFAM-15.0
source package 0dd71e29-8eb1-4512-b37c-42f7158718f4 
interproscan.sh -i analysis_and_temp_files/05_genome_annotation/SL0000003_pred/predict_results/Trebouxia_SL0000003_SL0000003.proteins.fa -d analysis_and_temp_files/05_genome_annotation/SL0000003_pred/interpro -dp -f XML -goterms -dra -cpu 20
```

* Funannotate annotate
```{r,eval =F}
singularity run ../singularity/funannotate.sif funannotate annotate -i analysis_and_temp_files/05_genome_annotation/SL0000003_pred/ --iprscan analysis_and_temp_files/05_genome_annotation/SL0000003_pred/interpro/Trebouxia_SL0000003_SL0000003.proteins.fa.xml --cpus 20  --sbt analysis_and_temp_files/05_genome_annotation/template.sbt --busco_db /tsl/data/busco_lineages/chlorophyta_odb10  --rename SL003
```
```
...
```{r,eval =F}
singularity run ../singularity/funannotate.sif funannotate compare -i analysis_and_temp_files/05_genome_annotation/GTX0536_pred2/ analysis_and_temp_files/05_genome_annotation/SL0000003_pred/ -o funannotate_compare --cpus 20 -d /opt/databases

mv funannotate_compare analysis_and_temp_files/05_genome_annotation/
```

## 7. Identifying missing meiosis gene
* Blasted HOP1 from Coccomyxa (XP_005651810 )
```{r,eval =F}
blastp -query analysis_and_temp_files/05_genome_annotation/hop1_genbank.fa -subject analysis_and_temp_files/05_genome_annotation/GTX0536_pred2/annotate_results/Trebouxia_sp._A48.proteins.fa -evalue 1e-5 -outfmt 6
XP_005651810.1  GTX0536PRED_002087-T1   41.667  180     103     2       24      201     1       180     2.65e-46        165
```

* HOP2 (XP_005643862.1)
```{r,eval =F}
blastp -query analysis_and_temp_files/05_genome_annotation/hop2_genbank.fa -subject analysis_and_temp_files/05_genome_annotation/GTX0536_pred2/annotate_results/Trebouxia_sp._A48.proteins.fa -evalue 1e-5 -outfmt 6
XP_005643862.1  GTX0536PRED_004154-T1   43.458  214     92      2       4       188     2       215     5.86e-53        167
```

* MER3 (XP_005651102)
```{r,eval =F}
blastp -query analysis_and_temp_files/05_genome_annotation/mer3_genbank.fa -subject analysis_and_temp_files/05_genome_annotation/GTX0536_pred2/annotate_results/Trebouxia_sp._A48.proteins.fa -evalue 1e-5 -outfmt 6
...
```{r,eval =F}
blastp -query analysis_and_temp_files/05_genome_annotation/mid1_genbank.fa -subject analysis_and_temp_files/05_genome_annotation/GTX0536_pred2/annotate_results/Trebouxia_sp._A48.proteins.fa -evalue 1e-5 -outfmt 6
...
* For MT+ specific FUS1, used Chlamydomonas reinhardtii (AAC49416.1)
  * no hits
```{r,eval =F}
blastp -query analysis_and_temp_files/05_genome_annotation/fus1_genbank.fa -subject analysis_and_temp_files/05_genome_annotation/GTX0536_pred2/annotate_results/Trebouxia_sp._A48.proteins.fa -evalue 1e-5 -outfmt 6
```
