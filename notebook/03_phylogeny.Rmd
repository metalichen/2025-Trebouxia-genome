...
blastn \
 -subject analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear_final_sort.fasta \
...
samtools faidx analysis_and_temp_files/05_genome_annotation/GTX0536_nuclear_final_sort.fasta GTX0536_15:2399373-2400063 > analysis_and_temp_files/03_phylogeny/GTX0536_its.fa
...
```{r,eval=F}
cp analysis_and_temp_files/05_genome_annotation/SL0000003_pred/annotate_results/Trebouxia_SL0000003_SL0000003.proteins.fa analysis_and_temp_files/03_phylogeny/orthofinder_input/

cp analysis_and_temp_files/05_genome_annotation/GTX0536_pred2/update_results/Trebouxia_sp._A48.proteins.fa analysis_and_temp_files/03_phylogeny/orthofinder_input/
```

#### Orthofinder: inital run
```{r,eval=F}
...
#### Re-ranning Orthofinder with the addition of T. lynnae
* Will add to our phylogenomic additional genome of Trebouxia lynnae 
* NCBI contains 3 assemblies, but no genome annotation is available
* Ran BUSCO on the genome JAVLRR01, which is the only one published
```{r,eval=F}
mkdir analysis_and_temp_files/03_phylogeny/Treblynn_busco
...
busco -i data/ref_genomes/GCA_042018865.1_Tlynv2_genomic.fna  -o Treblynn --out_path analysis_and_temp_files/03_phylogeny/Treblynn_busco  -m  genome -l /tsl/data/busco_lineages/chlorophyta_odb10 --offline -f -c 20
```
* Saved all fastas into a single file
```{r,eval=F}
cat analysis_and_temp_files/03_phylogeny/Treblynn_busco/Treblynn/run_chlorophyta_odb10/busco_sequences/*/*.faa > analysis_and_temp_files/03_phylogeny/Treblynn_busco/Treblynn_busco.faa
```

* Re-ran Orthofinder with the new genome
```{r,eval=F}
source package fc91613f-1095-4f67-b5aa-b86d702b36da
/software/fc91613f-1095-4f67-b5aa-b86d702b36da/bin/orthofinder -f analysis_and_temp_files/03_phylogeny/Treblynn_busco -b analysis_and_temp_files/03_phylogeny/orthofinder_input/OrthoFinder/Results_Apr13/WorkingDirectory
```
```
OrthoFinder assigned 289558 genes (90.4% of total) to 19599 orthogroups. Fifty percent of all genes were in orthogroups with 25 or more genes (G50 was 25) and were contained in the largest 4068 orthogroups (O50 was 4068). There were 328 orthogroups with all species present and 36 of these consisted entirely of single-copy genes.

328 trees had all species present and will be used by STAG to infer the species tree
```
...
colnames(ortho)[colnames(ortho)=="Treblynn_busco" ] <- "Trebouxia lynnae"
